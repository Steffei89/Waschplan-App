            ...
            if (userDocSnap.exists()) {
                ...
                if (userData.partei) {
                    try {
                        ...
                        console.error("Karma Fehler (wird ignoriert):", karmaError);
                    }
                }
            ...
            bookings.forEach(booking => {
                ...
                if (isMyParteiBooking || userIsAdmin) {
                    ...
                    if (userIsAdmin) actionsDiv.appendChild(delBtn);
                        ...
                        if (!booking.checkInTime) actionsDiv.appendChild(delBtn);
                    }
    ...
    const unsub = loadIncomingRequests(
        (pendingRequests) => {
            ...
            pendingRequests.forEach(req => {
                ...
                acceptBtn.className = 'button-small button-success';
                acceptBtn.textContent = 'Annehmen';
                acceptBtn.onclick = (e) => {
                    e.preventDefault(); e.stopPropagation();
                    acceptBtn.disabled = true; acceptBtn.textContent = 'Prüfe...';
                    confirmSwapTransaction(req.id);
                };
                const rejectBtn = document.createElement('button');
                rejectBtn.className = 'button-small button-secondary';
                rejectBtn.textContent = 'Ablehnen';
                rejectBtn.onclick = (e) => {
                     e.preventDefault(); e.stopPropagation();
                     rejectBtn.disabled = true; rejectBtn.textContent = '...';
                     rejectSwapRequest(req.id);
                };
        ...
        (error) => {
            dom.incomingRequestsContainer.style.display = 'block'; 
            dom.incomingRequestsContainer.innerHTML = '<p class="message-box error">Fehler beim Laden.</p>';
        }
    );
...
function handleLoadOutgoingRequests() {
    const unsub = loadOutgoingRequestStatus(
        (rejectedRequests) => {
            ...
            header.textContent = `Abgelehnte Anfragen (${rejectedRequests.length})`;
            ...
            rejectedRequests.forEach(req => {
                ...
                okBtn.className = 'button-small button-secondary dismiss-notification-btn';
                okBtn.textContent = 'OK';
                okBtn.addEventListener('click', async (e) => {
                    e.preventDefault();
                    okBtn.disabled = true;
                    await dismissRequestNotification(req.id);
                });
        ...
        (error) => console.error("Fehler ausgehende Anfragen:", error)
    );
...
function handleLoadOutgoingSuccess() {
    const unsub = loadOutgoingRequestSuccess(
        (acceptedRequests) => {
            ...
            header.textContent = `Angenommene Anfragen (${acceptedRequests.length})`;
            container.appendChild(header);
        ...
        (error) => console.error("Fehler ausgehende Erfolgs-Anfragen:", error)
    );
    setUnsubscriber('outgoingRequestsSuccess', unsub);
}

function handleLoadPrograms() {
    const unsub = loadWashPrograms(
        (programs) => {
            allPrograms = programs;
            renderTimerUI(); 
        },
        (error) => console.error("Fehler beim Laden der Waschprogramme:", error)
    );
...
function handleListenToTimer() {
    ...
    const unsub = listenToActiveTimer(partei, (timerData) => {
...
async function getStoredQrCode() {
    try {
        ...
        if (snap.exists() && snap.data().qrCodeSecret) {
            return snap.data().qrCodeSecret;
        }
        return 'WASCH-START'; 
...
function renderTimerUI() {
    ...
    if (currentTimerData) {
        ...
        dom.liveTimerSection.innerHTML = `
            <div class="timer-running-container">
                <h3>Programm läuft: ${programName}</h3>
                <div class="progress-bar-container">
                    <div class="progress-bar-fill" id="timer-progress-fill"></div>
                </div>
                <p class="timer-countdown-text" id="timer-countdown-text">Berechne...</p>
                <button class="timer-stop-btn" id="timer-stop-btn"><i class="fa-solid fa-right-from-bracket"></i> Check-out / Stopp</button>
            </div>
        `;
        ...
        const updateCountdown = () => {
            ...
            if(countdownTextEl) {
                countdownTextEl.textContent = `${remainingMinutes}:${String(remainingSeconds).padStart(2, '0')} Min. verbleibend`;
            }
                ...
                startScanner(
                    (scannedCode) => {
                        if (scannedCode === correctCode) {
                            stopWashTimer(currentUser.userData.partei);
                            showMessage('main-menu-message', 'Check-out erfolgreich! ✅', 'success');
                        } else {
        ...
        if (myCurrentBooking) {
            ...
            if (!myCurrentBooking.checkInTime) {
                ...
                if(allPrograms.length > 0) {
                    buttonsHTML = '<p style="margin-top:15px; font-weight:bold;">Programm-Timer starten (optional):</p><div class="timer-button-row">' + 
                    allPrograms.map(prog => 
                        `<button class="timer-start-btn" data-id="${prog.id}">${prog.name} (${prog.durationMinutes} min)</button>`
                    ).join('') + '</div>';
                }

                dom.liveTimerSection.innerHTML = `
                    <div class="timer-start-container">
                        <div style="background:var(--success-color); color:white; padding:5px 10px; border-radius:5px; display:inline-block; margin-bottom:10px; font-size:0.9em;">
                            <i class="fa-solid fa-check"></i> Eingecheckt seit ${new Date(myCurrentBooking.checkInTime).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}
                        </div>
                        <button id="check-out-btn" class="button-danger">
                            <i class="fa-solid fa-right-from-bracket"></i> Check-out / Fertig
                        </button>
                        ${buttonsHTML}
                    </div>
                `;
                ...
                document.getElementById('check-out-btn').addEventListener('click', async () => {
                    if(confirm("Wäsche fertig? Slot freigeben?")) {
                        await performCheckOut(myCurrentBooking.id, 'main-menu-message');
                    }
                });
                ...
                document.querySelectorAll('.timer-start-btn').forEach(btn => {
                    btn.addEventListener('click', async (e) => {
                        ...
                        if(prog) {
                            startWashTimer(currentUser.userData.partei, prog);
                        }
                    });
                });
            }
...
function checkPasswordMatch() {
    ...
    const confirmValue = regPassConfirm.value;
    if (passValue === "" && confirmValue === "") {
        regPass.classList.remove('input-valid', 'input-invalid');
        regPassConfirm.classList.remove('input-valid', 'input-invalid');
        return;
    }
    if (passValue === confirmValue) {
        regPass.classList.add('input-valid');
        regPass.classList.remove('input-invalid');
        regPassConfirm.classList.add('input-valid');
        regPassConfirm.classList.remove('input-invalid');
    } else {
        regPass.classList.add('input-invalid');
        regPass.classList.remove('input-valid');
        regPassConfirm.classList.add('input-invalid');
...
document.getElementById('show-register').addEventListener('click', () => {
    ...
    if (regPassConfirm) {
        ...
        regPassConfirm.classList.remove('input-valid', 'input-invalid');
        regPassConfirm.addEventListener('input', checkPasswordMatch);
    }
    if (inviteCodeField) {
        inviteCodeField.value = '';
        inviteCodeField.addEventListener('input', checkInviteCode); 
    }
    if (parteiWrapper) {
        parteiWrapper.classList.add('hidden'); 
        const parteiSelect = document.getElementById('register-partei');
...
document.getElementById('refresh-app-btn').addEventListener('click', () => {
    ...
    setTimeout(() => btn.classList.remove('fa-spin'), 1500);
});
...
function setupMainMenuListeners() {
    ...
    if (reportBtn) {
        reportBtn.addEventListener('click', () => {
            ...
            if (maintSec) navigateTo(maintSec);
        });
    }
        ...
        if (success) {
            ...
            setTimeout(() => {
                ...
                if(bookText) {
                    ...
                    bookText.textContent = originalText; 
                }
...
dom.changelogCloseBtn.addEventListener('click', () => {
